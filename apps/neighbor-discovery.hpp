 ...
 */
class NeighborDiscovery : public ndn::App {
public:
...
protected:
  ...
  virtual void
  SendDataWithDelay(std::shared_ptr<const ndn::Data> data);
...
protected:
  ...
  uint8_t m_maxHops; // number of max hops
  double m_maxDelay;
  std::tuple<double, double, double> m_prevLocation;  //previous location sent
  std::tuple<double, double, double> m_prevLocationCopy;  //previous location sent
  ndn::time::steady_clock::time_point m_prevTime; 
};
